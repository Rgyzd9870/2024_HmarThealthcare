...
void iic_init(void)
{
    ...
    /* SCL、SDA引脚设置为开漏输出, 这样就不用再设置IO方向了, 开漏输出的时候(=1), 也可以读取外部信号的高低电平 */
    ...
    GPIO_Init(IIC_SCL_GPIO_PORT, &gpio_init_struct);    /* SCL */

    gpio_init_struct.GPIO_Pin = IIC_SDA_GPIO_PIN;
    GPIO_Init(IIC_SDA_GPIO_PORT, &gpio_init_struct);    /* SDA */
