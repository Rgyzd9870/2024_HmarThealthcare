...
void LCD_GPIO_Init(void)
{
	...
	RCC_APB2PeriphClockCmd( RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOC ,ENABLE);//配置时钟
	...
	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_5;                       //PC5  命令/数据--切换引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;                //普通输出模式
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;               //100MHz
...
{		 			 
  ...
  while((SPI2->STATR & SPI_I2S_FLAG_TXE) == RESET);  //while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET){}//等待发送区空
	
	SPI2->DATAR = TxData;                               //SPI_I2S_SendData(SPI2, TxData); //通过外设SPIx发送一个byte  数据
	
  while((SPI2->STATR & SPI_I2S_FLAG_RXNE) == RESET);  //while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE) == RESET){} //等待接收完一个byte
 
	return SPI2->DATAR;                                 //SPI_I2S_ReceiveData(SPI2); //返回通过SPIx最近接收的数据
...
//	Draw_Font24B(16,4,MAGENTA,"DevEBox-大越创新");
	Draw_Font24B(52,296,MAGENTA,"共创芯未来");

	POINT_COLOR = GREEN;      //画笔颜色：红色
	Draw_Font24B(52,100,MAGENTA,"心率:");
	Draw_Font24B(52,150,MAGENTA,"血氧:");
	Draw_Font24B(52,200,MAGENTA,"老人摔倒:");
	DisplayButtonUp(52,100,100,124); //x1,y1,x2,y2
	DisplayButtonUp(52,150,100,174); //x1,y1,x2,y2
	DisplayButtonUp(52,200,148,224); //x1,y1,x2,y2
